...
负责装配会话主链路，包含：
- DeerFlow 运行时作用域（runtime_root 可按请求注入）
- 财务工具上下文注册（使用 open_context_scope 实现请求级生命周期）
- 依赖装配
...
class ConversationRouterFactory:
    ...
    - 工具上下文通过 open_context_scope() 在 AppConversationHandler.handle() 中实现请求级作用域
    ...
    def __init__(
        ...
        Args:
            ...
            cashier_repository: 出纳仓储。
            runtime_root: DeerFlow 运行时根目录。
    ...
    def _build_router_and_context(self):
        ...
        conversation_service = ConversationService(
            finance_department_service=self._orchestration_bundle.finance_department_service,
            reply_text_sanitizer=ReplyTextSanitizer(),
        )
        router = ConversationRouter(conversation_service)
        return router, finance_tool_context
