...
- 产品定位：小企业本地私有部署的智能财务部门，当前主路径是会计期间、凭证生命周期、账簿报表 + 出纳/银行基础能力
...
- 确定性业务 API 直接调用 `AccountingService` / `CashierService`，不经过 LLM
...
- `accounting/`：会计科目、期间、凭证生命周期、账簿报表
- `audit/`：凭证审核
...
- 改 API 契约时，回归：
  ...
  - `tests.test_production_api_endpoints`
- 改会计期间、凭证生命周期、报表或迁移时，回归：
  - `tests.test_accounting_production_core`
- 改工作台摘要或回复压缩逻辑时，回归：
