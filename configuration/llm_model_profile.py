...
DEFAULT_MODEL_USE_PATH = "langchain_openai:ChatOpenAI"
...
class LlmModelProfile:
    """描述一个可供 DeerFlow 运行时使用的模型条目。

    这里不再把“当前系统只有一个活跃模型”的假设写死在配置层，而是把每个模型
    的运行参数、环境变量名和能力标记显式建模。这样做有三个目的：

    1. 让项目配置结构和 DeerFlow `config.yaml` 的 `models:` 列表保持同构；
    ...
    Attributes:
        ...
        api_key_env: DeerFlow 运行时应读取的环境变量名。
        api_key: 当前进程内解析出的真实密钥，用于在运行前写入环境变量。
        display_name: 用户可见展示名；为空时退化为 `model_name`。
        use_path: DeerFlow/ LangChain 解析模型类时使用的导入路径。
